...
//            }
    ...
    private static HttpResponse readResponse(HttpURLConnection con) throws IOException {
	    try {
		    return httpResponse(con, false);
	    } catch (Exception e) {
		    return httpResponse(con, true);
	    } finally {
		    con.disconnect();
	    }
    }

	private static HttpResponse httpResponse(HttpURLConnection con, boolean isError) throws IOException {
		BufferedReader br;
		if (isError) {
    ...
    public static void main(String[] args) throws IOException {
        String url = "http://test.nextop.cc/gw/diplomat/irobotbox/order";

        HttpResponse response = getClient(url)
                .post()              //设置post请求
                .addHeader("Content-Type", "application/json")   //添加header
                .body("{\"userName\":\"data009\",\"password\":\"data009\",\"customerId\":1837,\"startTime\":\"2019-10-22 00:00:00\",\"endTime\":\"2019-10-22 23:00:00\",\"orderListStatus\":\"All\",\"sellerOrderId\":\"114-8982738-1409032\"}")
                .request();         //发起请求
